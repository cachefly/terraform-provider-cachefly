...
resource "cachefly_origin" "example" {
...
func (d *ServiceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if hasID {
		// Look up by ID using the appropriate SDK method
		...
		listOptions := api.ListOptions{
			// We can set some reasonable defaults for the list operation
			Limit: 1000, // Get a large batch to avoid pagination issues
		}
